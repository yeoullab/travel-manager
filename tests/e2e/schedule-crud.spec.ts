...
test.describe("일정 CRUD (Spec §2.5)", () => {
  test("trip 생성 → 일정 추가 (장소 없음) → 새로고침 후 persist", async ({ page }) => {
    ...
    await expect(page.getByText("첫 일정", { exact: true })).toBeVisible({ timeout: 5_000 });

    await page.reload();
    await expect(page.getByText("첫 일정", { exact: true })).toBeVisible();
  });

  test("일정 편집 → 삭제 → 새로고침 후 persist", async ({ page }) => {
    await page.goto(`/trips/${tripId}`);

    await page.getByText("첫 일정", { exact: true }).click();
    ...
    await expect(page.getByText("첫 일정 (수정)", { exact: true })).toBeVisible({ timeout: 5_000 });

    page.once("dialog", (d) => d.accept());
    await page.getByText("첫 일정 (수정)", { exact: true }).click();
    await expect(page.getByText(/일정 \(기타\)/)).toBeVisible({ timeout: 5_000 });
    await page.getByRole("button", { name: "삭제" }).click();

    await expect(page.getByText("첫 일정 (수정)", { exact: true })).toHaveCount(0, { timeout: 5_000 });

    await page.reload();
    await expect(page.getByText("첫 일정 (수정)", { exact: true })).toHaveCount(0);
  });
});
