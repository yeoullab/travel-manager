import { afterAll, beforeAll, describe, expect, it } from "vitest";
import { createClient, type SupabaseClient } from "@supabase/supabase-js";
import { env, getServerEnv } from "@/lib/env";
import type { Database } from "@/types/database";

const { SUPABASE_SERVICE_ROLE_KEY } = getServerEnv();
const admin = createClient<Database>(env.NEXT_PUBLIC_SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  auth: { persistSession: false },
});
const STAMP = Date.now();
const PWD = "Test_Pwd_2026!";
let aliceId = "";
let bobId = "";
let evelynId = "";
let aliceC: SupabaseClient<Database>;
let bobC: SupabaseClient<Database>;
let evelynC: SupabaseClient<Database>;
let tripId = "";
let todoId = "";

beforeAll(async () => {
  for (const [role, keep] of [
    ["alice", (id: string) => { aliceId = id; }],
    ["bob", (id: string) => { bobId = id; }],
    ["evelyn", (id: string) => { evelynId = id; }],
  ] as const) {
    const { data, error } = await admin.auth.admin.createUser({
      email: `${role}rtodo+${STAMP}@test.local`,
      password: PWD,
      email_confirm: true,
    });
    if (error) throw error;
    keep(data.user!.id);
  }
  const mkClient = async (role: string) => {
    const c = createClient<Database>(
      env.NEXT_PUBLIC_SUPABASE_URL,
      env.NEXT_PUBLIC_SUPABASE_ANON_KEY,
      { auth: { persistSession: false } },
    );
    await c.auth.signInWithPassword({
      email: `${role}rtodo+${STAMP}@test.local`,
      password: PWD,
    });
    return c;
  };
  aliceC = await mkClient("alice");
  bobC = await mkClient("bob");
  evelynC = await mkClient("evelyn");

  const { data: inv } = await aliceC.rpc("create_invite");
  await bobC.rpc("accept_invite", { p_invite_code: (inv as { invite_code: string }).invite_code });

  const r = await aliceC.rpc("create_trip", {
    p_title: "Todo RLS",
    p_destination: "Fukuoka",
    p_start_date: "2026-06-01",
    p_end_date: "2026-06-02",
    p_is_domestic: false,
    p_currencies: ["JPY"],
  });
  if (r.error) throw r.error;
  tripId = r.data as string;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const t = await (aliceC as any).rpc("create_todo", {
    p_trip_id: tripId,
    p_title: "Pack passport",
    p_memo: null,
    p_assigned_to: null,
  });
  if (t.error) throw t.error;
  todoId = t.data as string;
}, 30_000);

afterAll(async () => {
  await admin.from("todos").delete().eq("trip_id", tripId);
  await admin.from("trips").delete().eq("id", tripId);
  await admin
    .from("group_members")
    .delete()
    .or(`user_id.eq.${aliceId},user_id.eq.${bobId},user_id.eq.${evelynId}`);
  await admin.from("groups").delete().eq("created_by", aliceId);
  for (const id of [aliceId, bobId, evelynId]) await admin.auth.admin.deleteUser(id);
});

describe("todos RLS (can_access_trip × CRUD)", () => {
  it("owner — SELECT OK", async () => {
    const { data, error } = await aliceC.from("todos").select("*").eq("id", todoId);
    expect(error).toBeNull();
    expect(data?.length).toBe(1);
  });

  it("member — SELECT OK", async () => {
    const { data, error } = await bobC.from("todos").select("*").eq("id", todoId);
    expect(error).toBeNull();
    expect(data?.length).toBe(1);
  });

  it("stranger — SELECT 0 row", async () => {
    const { data } = await evelynC.from("todos").select("*").eq("id", todoId);
    expect(data?.length).toBe(0);
  });

  it("stranger — INSERT via RPC 거부", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (evelynC as any).rpc("create_todo", {
      p_trip_id: tripId,
      p_title: "Intruder",
      p_memo: null,
      p_assigned_to: null,
    });
    expect(error).not.toBeNull();
  });

  it("stranger — UPDATE via RPC 거부", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (evelynC as any).rpc("update_todo", {
      p_todo_id: todoId,
      p_title: "Hacked",
      p_memo: null,
      p_assigned_to: null,
    });
    expect(error).not.toBeNull();
  });

  it("stranger — toggle 거부", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (evelynC as any).rpc("toggle_todo", {
      p_todo_id: todoId,
      p_complete: true,
    });
    expect(error).not.toBeNull();
  });

  it("stranger — DELETE via RPC 거부", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (evelynC as any).rpc("delete_todo", { p_todo_id: todoId });
    expect(error).not.toBeNull();
  });

  it("member — UPDATE OK", async () => {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { error } = await (bobC as any).rpc("update_todo", {
      p_todo_id: todoId,
      p_title: "Pack passport (partner edit)",
      p_memo: null,
      p_assigned_to: null,
    });
    expect(error).toBeNull();
  });
});
