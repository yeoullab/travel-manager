...
### Font Family
- **Korean UI Primary**: `Pretendard Variable`, with fallbacks: `Pretendard, -apple-system, BlinkMacSystemFont, system-ui, Roboto, Helvetica Neue, Arial, sans-serif`
- **Display/Headlines (Latin)**: `CursorGothic`, with fallbacks: `CursorGothic Fallback, Pretendard Variable, system-ui, Helvetica Neue, Helvetica, Arial`
- **Body/Editorial**: `jjannon`, with fallbacks: `Pretendard Variable, Iowan Old Style, Palatino Linotype, URW Palladio L, P052, ui-serif, Georgia, Cambria, Times New Roman, Times`
...
- **Korean-first dual-font strategy**: Pretendard Variable handles Korean UI copy (body, buttons, labels, form inputs) as the primary voice. CursorGothic remains for Latin-script display headings, brand moments, and numerics where the compressed tracking signature matters. jjannon for editorial/long-form passages via `font-family: jjannon, "Pretendard Variable", serif` stacking so Korean falls back to Pretendard within editorial contexts. Weight scale for Pretendard: 400 (body), 500 (labels/buttons), 600 (headings), 700 (emphasis) — honoring Pretendard's variable axis while keeping Cursor's weight restraint.
...
### Touch Targets
- **Minimum hit area: 44×44pt (Apple HIG)** — applies to every interactive element on mobile breakpoints; Android target 48×48dp where relevant
- Buttons use comfortable padding (6px-14px vertical, 8px-14px horizontal); on mobile, vertical padding extends so total height reaches 44pt even when label is small
- Pill buttons maintain tap-friendly sizing with 3px-10px padding; on mobile, wrap in an invisible 44pt hit container if the visual pill is smaller
- Navigation links at 14px with adequate spacing for touch; inline links within body copy use vertical padding ≥12px to avoid mis-taps
- Minimum spacing between adjacent interactive targets: 8px (prevents fat-finger mis-selection)
...
- "Build a Bottom Sheet modal for adding an expense. Background `#f2f1ed`, top corners 16px radius, drag handle 40×4pt pill in `oklab(0.263 / 0.2)` with 8px top and 12px bottom margin. Title 17px Pretendard weight 600. Form fields stacked. Sticky full-width primary CTA at bottom, height 48pt, bg `#ebeae5`. Backdrop `rgba(38, 37, 30, 0.55)` with blur(2px). Enter animation slide-up 300ms ease-out. Include `padding-bottom: env(safe-area-inset-bottom) + 16px`."
- "Render a skeleton card list for loading state. 3 cards stacked, each with 8px radius and `#ebeae5` base. Inside each card: 40×40pt circle avatar skeleton, two text skeleton lines (width 70% then 50%, height 12px, radius 4px). Apply shimmer: `linear-gradient(90deg, #ebeae5, #e6e5e0, #ebeae5)` with background-position animation 1500ms linear infinite."
- "Compose a mobile app shell with Bottom Tab Bar (5 tabs: Schedule, Expenses, Todos, Notes, Settings) and FAB. Tab bar 56pt + safe-area-bottom, bg `#f2f1ed` with backdrop-filter blur(8px), 1px top border in `oklab(0.263 / 0.1)`. Inactive tabs: icon 24pt + 11px label in `rgba(38, 37, 30, 0.55)`; active tab: accent orange `#f54e00` with label weight 500 and scale(1.05). FAB anchored bottom-right 16px above tab bar, 56×56pt circle, bg `#f54e00`, icon `#f2f1ed`, Level 3 shadow."
...
4. Four fonts, four voices: Pretendard Variable (Korean UI primary), CursorGothic (Latin display/numerics), jjannon (editorial), berkeleyMono (code)
...
## 10. Mobile App Patterns

Design tokens and component contracts that extend the Cursor-inspired web system into a mobile-first PWA. **This section defines values only**; when and why each token applies is documented in the project spec `§7 UI/UX Design Direction` — see those UX policy notes for usage rules.

### 10.1 Safe Area & Viewport

- HTML `<meta>` requires `viewport-fit=cover` so iOS respects safe-area insets
- Page root applies `padding-top: max(16px, env(safe-area-inset-top))` and `padding-bottom: max(0px, env(safe-area-inset-bottom))` where needed
- Bottom-fixed surfaces (Tab Bar, Toast, Bottom Sheet, FAB) **must** include `padding-bottom: env(safe-area-inset-bottom)` or equivalent positioning offset
- Horizontal insets (`left/right`) only needed for landscape orientation on notched devices; use `env(safe-area-inset-left/right)` on full-width sticky bars

### 10.2 Touch Targets (Mobile Context)

See `§8 Touch Targets` for the full rule set. Key mobile additions:
- Enforce the 44×44pt minimum in hit area even when the visible control is smaller (wrap with invisible padding or `::before` pseudo-element)
- Never place two interactive targets closer than 8px edge-to-edge

### 10.3 Bottom Sheet

Modal pattern for mobile CRUD (contact/expense/schedule input, filters, option menus).

| Property | Value |
|----------|-------|
| Background | `#f2f1ed` (Cursor Cream) |
| Top corners | `border-radius: 16px 16px 0 0` |
| Drag handle | `40×4pt` pill, color `oklab(0.263 / 0.2)`, margin `8px auto 12px` |
| Backdrop | `rgba(38, 37, 30, 0.55)` with `backdrop-filter: blur(2px)` |
| Enter motion | `sheet-enter` + `backdrop-fade` (see §10.7) |
| Exit | reverse of enter, 200ms ease-in |
| Bottom inset | `padding-bottom: env(safe-area-inset-bottom) + 16px` |
| Primary CTA | sticky bottom, full-width minus 16px horizontal, height 48pt |
| Dismiss | backdrop tap / drag-down ≥100px / ESC key |
| Max height | `85vh` with inner scroll |

### 10.4 Skeleton Loaders

Placeholder shapes rendered before data arrives (Shell-First strategy per spec).

- **Base color:** Surface 300 `#ebeae5`
- **Shimmer color:** Surface 400 `#e6e5e0`
- **Gradient sweep:** `linear-gradient(90deg, #ebeae5 0%, #e6e5e0 50%, #ebeae5 100%)` animated via `background-position: -200% 0 → 200% 0` over `1500ms linear infinite`
- **Corner radius:** match the real component (card 8px, list row 4px, avatar 9999px)
- **Primitive shapes:**
  - `skeleton-text`: height `1em`, width `60–90%`, radius `2px`
  - `skeleton-line`: height `12px`, width `100%`, radius `4px`
  - `skeleton-card`: full card dimensions with nested text skeletons
  - `skeleton-avatar`: `40×40pt` circle

### 10.5 Pull-to-Refresh

- **Activation distance:** 60px downward drag from scroll top
- **Indicator:** 30×30pt spinner, stroke 2.5pt, color accent orange `#f54e00`, rotation `1s linear infinite` once triggered
- **Position:** horizontally centered, vertical offset tracks drag distance up to 80px max
- **Haptic:** on activation, call `navigator.vibrate(10)`; no-op if unsupported
- **States:** `idle → pulling → ready (pass threshold) → refreshing → done (fade out 200ms)`

### 10.6 Swipe Gestures

- **Left swipe → delete** (schedule / expense / todo items)
  - Activation distance: 80px leftward drag
  - Underlay reveal: background `#cf2d56` (error crimson) with white trash icon 24×24pt, right-aligned with 16px padding
  - Spring-back: `cubic-bezier(0.2, 0.8, 0.2, 1)` 250ms when cancelled
  - Confirm: drag ≥ `item-width / 2` commits delete; else snap back
- **Right swipe:** reserved for future use (not assigned in V1)

### 10.7 Motion Tokens

Values only. Application rules (which interactions use which token, why they exist) live in spec §7 Micro-interactions.

| Token | Value |
|-------|-------|
| `press-scale` | `transform: scale(0.97); transition: transform 100ms ease-out` |
| `drag-lift` | `transform: scale(1.03) rotate(-1.5deg); opacity: 0.95; box-shadow: level-3` |
| `slide-up-enter` | `translateY(20px), opacity 0 → translateY(0), opacity 1; 250ms ease-out` |
| `fade-in` | `opacity 0 → 1; 150ms ease-out` |
| `sheet-enter` | `translateY(100%) → translateY(0); 300ms ease-out` |
| `backdrop-fade` | `opacity 0 → 1; 200ms ease-out` |
| `toast-hold` | `3000ms` |
| `toast-exit` | `opacity 1 → 0; 150ms ease-out` |
| `spring-back` | `cubic-bezier(0.2, 0.8, 0.2, 1); 250ms` |

### 10.8 Mobile Common Components

| Component | Specification |
|-----------|---------------|
| **FAB** | 56×56pt circle, bg `#f54e00` (accent orange), icon `#f2f1ed` (cream), anchored bottom-right `16px + env(safe-area-inset-bottom)`, shadow Level 3 (Elevated Card), press uses `press-scale` |
| **Bottom Tab Bar** | Height 56pt + safe-area-bottom, bg `#f2f1ed` with `backdrop-filter: blur(8px)`, top border `1px solid oklab(0.263 / 0.1)`, 5 tabs max; active tab: icon + label in accent orange `#f54e00`, label weight 500, gentle `scale(1.05)`; inactive: color `rgba(38, 37, 30, 0.55)` weight 400 |
| **Sticky Header** | Height 56pt, bg `#f2f1ed` with `backdrop-filter: blur(8px)`, title 17px Pretendard weight 600 centered or left-aligned, back-chevron hit area 44×44pt on the leading edge; on scroll past 8px, fade in `border-bottom: 1px solid oklab(0.263 / 0.1)` over 150ms |
| **Toast** | Anchored bottom with `16px + env(safe-area-inset-bottom)` offset, max-width `min(400px, calc(100vw - 32px))`, bg `#26251e`, text `#f2f1ed` at 15px Pretendard weight 500, radius 16px, padding `12px 16px`, Level 2 ambient shadow; enter `slide-up-enter`, hold `toast-hold`, exit `toast-exit` |
| **Dialog (Confirm)** | Centered, `max-width: 340pt`, bg `#f2f1ed`, radius 16px, Level 3 shadow, padding 24px; title 18px Pretendard weight 600, body 15px weight 400 color `rgba(38, 37, 30, 0.75)`, actions right-aligned with 12px gap; destructive primary uses `#cf2d56` text on `#ebeae5` bg |
| **Empty State** | Vertical center, icon 48pt in `rgba(38, 37, 30, 0.35)`, primary line 17px Pretendard weight 500 `#26251e`, secondary line 14px weight 400 `rgba(38, 37, 30, 0.55)` with 4px top margin, CTA button 24px top margin (structure mirrors spec §7 Empty State Pattern) |
